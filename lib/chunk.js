...
module.exports = {
	decompress: async function (data) {
		...
		const offset = data[8];
		if (offset == 120) {
			var data = data.slice(8); //slice = substr (startOffset, endOffset)
		} else {
			var data = data.slice(offset); //the correct one might be 62
		}
		try {
	...
	//blobs are stored in sets of 3 digits, backwards.. this function parses them by spliting them and converting them to a hex string
