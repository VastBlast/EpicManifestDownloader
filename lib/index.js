...
exports.Downloader = class Downloader {
    ...
    downloadmanifest = async function () {
        await axios.post(
            ...
            let CC_TOKEN = data.access_token;
            await axios.get(
                ...
                let BASE_URL = MANIFEST.elements[0].manifests[0].uri;
                let PARAM = MANIFEST.elements[0].manifests[0].queryParams[0].value;
                let FULL_URL = `${BASE_URL}?ak_token=${PARAM}`;
                await axios.get(
                    FULL_URL
                ).then(function (response) {
                    let LATEST_MANIFEST = JSON.stringify(response.data);
                    fs.writeFileSync("./manifest.json", LATEST_MANIFEST);
                }).catch(function (error) {
                    throw 'An Error occured while downloading manifest';
                })
            }).catch(function (error) { // Shouldn't be an error
                throw 'Error getting Manifest';
            })
        }).catch(function (error) { // There shouldn't be an error anyways
            throw 'Error getting client credentials token';
        })
    }
